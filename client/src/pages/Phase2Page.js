...
function buildDisplayStateMap(nodes, timeline) {
    const eventsByNode = {};
...
export function Phase2Page() {
    const [sidebarMode, setSidebarMode] = useState("tree");
    ...
    const pastNodeQueries = useQueries({
        queries: pastDepths.map((d) => ({
            queryKey: ["layer-nodes", d],
            queryFn: () => apiFetch(`/phase2/layer/${d}/nodes`),
            staleTime: 5 * 60 * 1000
        }))
    });
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Header, { phase: "phase 2", label: `layer ${depth} · ${stepQ.status === "deriving" ? "..." : (stepQ.step ?? "idle")}`, llmBusy: false, states: allStates }), _jsxs("main", { className: "main layout-phase2", children: [_jsx(Sidebar, { mode: sidebarMode, setMode: setSidebarMode, children: sidebarMode === "tree" ? (_jsx(LayerTree, { stackLayers: stackLayers, groupedNodes: groupedNodes, displayStates: displayStates, selectedNodeId: selectedNodeId, onSelectNode: (id) => {
                                ...
                                setActiveTab("node");
                            } })) : (_jsx(EventStream, { events: timeline })) }), _jsx(DagCanvas, { groupedNodes: groupedNodes, displayStates: displayStates, selectedNodeId: selectedNodeId, selectedEdgeId: selectedEdge?.id ?? null, onSelectNode: (id) => {
                            setSelectedNodeId(id);
                            if (id)
                                setActiveTab("node");
                        }, onSelectEdge: (edge) => {
                            ...
                            if (edge)
                                setActiveTab("node");
                        } }), _jsx(RightPanel, { activeTab: activeTab, setActiveTab: setActiveTab, depth: depth, step: stepQ.step, stepStatus: stepQ.status, nodes: currentNodes, selectedNode: selectedNode, selectedEdge: selectedEdge, displayStates: displayStates, spec: specQ.data?.spec ?? null, definition: stepQ.layerDefinition.data?.definition ?? null, timeline: timeline, onDiagnosed: () => setActiveTab("step") })] })] }));
}
