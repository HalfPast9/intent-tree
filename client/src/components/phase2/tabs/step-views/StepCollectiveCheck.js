...
export function StepCollectiveCheck({ depth, parentIds }) {
    ...
    return (_jsxs("div", { style: { display: "grid", gap: 8 }, children: [_jsxs("div", { className: "mono", style: { fontSize: 10, color: "var(--tx2)" }, children: ["COLLECTIVE CHECK \u00B7 L", depth] }), !coverage && (_jsx("button", { className: "btn", onClick: () => void run(), disabled: collective.isPending, children: collective.isPending ? _jsx(Spinner, {}) : "run collective check" })), coverage && (_jsxs("div", { style: { display: "grid", gap: 6 }, children: [coverage.map((item) => (_jsxs("div", { className: "panel", style: { padding: 8, background: "var(--s2)", borderColor: item.covered ? "var(--bdr)" : "var(--proposed)" }, children: [_jsxs("div", { style: { display: "flex", justifyContent: "space-between" }, children: [_jsx("span", { className: "mono", style: { fontSize: 10 }, children: item.parent_id }), _jsx("span", { className: "mono", style: { fontSize: 10, color: item.covered ? "var(--passed)" : "var(--failed)" }, children: item.covered ? "✓ covered" : "✕ gap" })] }), !item.covered && item.gap && _jsx("div", { style: { fontSize: 11, color: "var(--tx2)", marginTop: 4 }, children: item.gap })] }, item.parent_id))), gapParents.length > 0 && (_jsxs("div", { style: { display: "grid", gap: 6, marginTop: 4 }, children: [gapParents.map((id) => (_jsx("button", { className: "btn", disabled: queued.has(id) || repropose.isPending, onClick: () => void handleRepropose(id), children: queued.has(id) ? "✓ queued" : `re-propose for ${id}` }, id))), queued.size > 0 && (_jsx("button", { className: "btn btn-pri", onClick: () => void handleApprove(), disabled: approve.isPending, children: approve.isPending ? _jsx(Spinner, {}) : "approve re-proposals" }))] })), gapParents.length === 0 && (_jsx("div", { style: { fontSize: 11, color: "var(--passed)" }, children: "\u2713 All parents covered \u2014 no gaps detected." })), _jsx("button", { className: "btn", onClick: () => void run(), disabled: collective.isPending, children: collective.isPending ? _jsx(Spinner, {}) : "re-run collective check" })] }))] }));
}
